...
def load_metadata(config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    train_df = pd.read_csv(f"../metadata/DanishFungi2020-Mini_train_metadata_DEV.csv")  # TODO .. > .

    valid_df = pd.read_csv(f"../metadata/DanishFungi2020-Mini_test_metadata_DEV.csv")  # TODO

    train_df["image_path"] = train_df.image_path.apply(
        lambda path: os.path.join(LOCAL_DIR, os.path.basename(path).replace(".jpg", ".JPG")))
